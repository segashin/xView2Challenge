...
def process_data(input_path, output_path_pre, output_path_post, output_csv_path_pre, output_csv_path_post, val_split_pct):
    """Process Raw Data into
    ...
    for img_path in tqdm(img_paths):
        ...
        for feat in label_data['features']['xy']:
            ...
            if "pre" in img_path:
                #print(output_path_pre)
                cv2.imwrite(output_path_pre + "/" + poly_uuid, poly_img)
                y_data_pre.append(damage_intensity_encoding[damage_type])
                x_data_pre.append(poly_uuid)
            else:
                cv2.imwrite(output_path_post + "/" + poly_uuid, poly_img)
                y_data_post.append(damage_intensity_encoding[damage_type])
                x_data_post.append(poly_uuid)
    
    output_train_csv_path_pre = os.path.join(output_csv_path_pre, "train_pre.csv")
    output_train_csv_path_post = os.path.join(output_csv_path_post, "train_post.csv")
    ...
    # else: 
    data_array_pre = {'uuid': x_data_pre, 'labels': y_data_pre}
    data_array_post = {'uuid': x_data_post, 'labels': y_data_post}
    df_pre = pd.DataFrame(data = data_array_pre)
    df_post = pd.DataFrame(data = data_array_post)
    # make sure df_pre and df_post are in same order
    df_pre = df_pre.sort_values(by=['uuid'])
    df_post = df_post.sort_values(by=['uuid'])

    df_pre.to_csv(output_train_csv_path_pre)
    df_post.to_csv(output_train_csv_path_post)
...
def main():
    ...
    parser.add_argument('--output_dir_pre',
                        ...
                        help="Path to new directory to save pre images")
    parser.add_argument('--output_dir_post',
                        required=True,
                        metavar='/path/to/xBD_output',
                        help="Path to new directory to save post images")
    parser.add_argument('--output_dir_csv_pre',
                        required=True,
                        metavar='/path/to/xBD_output_csv',
                        help="Path to new directory to save pre-disaster csv")
    parser.add_argument('--output_dir_csv_post',
                        required=True,
                        metavar='/path/to/xBD_output_csv',
                        help="Path to new directory to save post-disaster csv")
    ...
    process_data(args.input_dir, args.output_dir_pre, args.output_dir_post, args.output_dir_csv_pre, args.output_dir_csv_post, float(args.val_split_pct))
